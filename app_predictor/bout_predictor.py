...
class FighterMatcher:
    ...
    def get_fighter_match(self, input_name: str) -> Optional[str]:
        """
        Automated version of name matching. 
        Matches an input name against the database and returns the best hit.
        """
        if not input_name or pd.isna(input_name):
            return None
            
        # Find matches using your existing _find_best_matches logic
        # Set a high threshold (e.g., 0.8) for automated matching to avoid wrong picks
        matches = self._find_best_matches(input_name, threshold=0.8, top_k=1)
        
        if matches:
            return matches[0][0] # Return the name of the top match
        
        return None # Or return input_name if you want to keep the original
    
    def run_batch_matching(self, input_df: pd.DataFrame) -> pd.DataFrame:
        """
        Processes an entire dataframe of fights and matches 
        red/blue names against the internal database.
        """
        print(f"=== Processing {len(input_df)} fights ===")
        
        # Create copies or new columns to store the matched names
        input_df['matched_red'] = input_df['fighter_red'].apply(self.get_fighter_match)
        input_df['matched_blue'] = input_df['fighter_blue'].apply(self.get_fighter_match)
        
        # Calculate success rate
        matched_count = input_df['matched_red'].notna().sum() + input_df['matched_blue'].notna().sum()
        total_expected = len(input_df) * 2
        
        print(f"✅ Matching complete. Matched {matched_count}/{total_expected} fighters.")
...
class ModelPredictor:
    ...
    def _predict_sklearn(self, model, fighter_differences):
        ...
        fighter_red_prob = 0.5
        fighter_blue_prob = 0.5
        ...
        try:
            ...
            fighter_red_prob = float(probs[red_idx])
            fighter_blue_prob = float(probs[blue_idx])
            ...
            try:
                ...
                decision_score = decision_scores[0] if isinstance(decision_scores, (list, np.ndarray)) else decision_scores
                
                # Normalize the decision score to probability-like values
                # Using sigmoid function to convert decision boundary to probability
                ...
                # Total fallback - use raw prediction
                prediction = raw_prediction
                fighter_red_prob = 0.99 if prediction == 1 else 0.01
                fighter_blue_prob = 1 - fighter_red_prob

        confidence = float(max(fighter_red_prob, fighter_blue_prob))
...
class UFCPredictionPipeline:
    ...
    def select_fighters_interactive(self) -> pd.DataFrame:
        ...
        filtered_data = matcher.run_batch_matching()
    ...
    def prepare_for_prediction(self, df: pd.DataFrame) -> Dict:
        ...
        prediction_row_selected = prediction_row_selected.fillna(0.0)
        ...
        # Final NaN check and fill before returning
        feature_dict = {k: (v if not np.isnan(v) else 0.0) for k, v in feature_dict.items()}
    ...
    def _encode_stances(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Fill NaN values in stances with the most common stance
        # Using 'Orthodox' as default since it's the most common UFC stance
        df['stance_red'] = df['stance_red'].fillna('Orthodox')
        df['stance_blue'] = df['stance_blue'].fillna('Orthodox')
    ...
    def run_batch_pipeline(self, input_csv: str, output_csv: str):
        ...
        for _, row in upcoming_fights.iterrows():
            # Step A: Create a context for this specific fight
            names = [row['fighter_red'], row['fighter_blue']]
            
            # Normalize the input fighter names
            normalized_names = [self.preprocessor.normalize_name(name) for name in names]
            
            # Filter historical data for these two fighters
            filtered_data = self.merged_data[
                (self.merged_data['fighter_red'].isin(normalized_names)) | 
                (self.merged_data['fighter_blue'].isin(normalized_names))
            ].copy()
            ...
            # Step B: Append the 'Upcoming' row using your existing function
            fight_with_prediction_row = self._add_prediction_row(filtered_data, normalized_names)
            
            # Manually ensure the last row has the correct metadata from our CSV
            fight_with_prediction_row.iloc[-1, fight_with_prediction_row.columns.get_loc('event_name')] = row['event_name']
            fight_with_prediction_row.iloc[-1, fight_with_prediction_row.columns.get_loc('weight_class')] = row['weight_class']

            # Step C: Feature Engineering & Prediction
            processed_data = self.engineer_features(fight_with_prediction_row)
            ...
            winner_name = row['fighter_red'] if prediction_label == 1 else row['fighter_blue']
        ...
        print(f"Successful: {len([r for r in all_results if r['predicted_winner'] != 0])}")
        print(f"Skipped: {len([r for r in all_results if r['predicted_winner'] == 0])}")
